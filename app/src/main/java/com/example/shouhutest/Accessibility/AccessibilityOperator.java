...
public class AccessibilityOperator {
    ...
    public void updateEvent(AccessibilityService service, AccessibilityEvent event) {
        ...
        if (service != null && mAccessibilityService == null) {
            ...
            Log.e(TAG, "updateEvent: service==null?"+(service==null) );
        }
        if (event != null) {
    ...
    private AccessibilityNodeInfo getRootNodeInfo() {
        ...
        if (Build.VERSION.SDK_INT >= 16) {
            ...
            if (mAccessibilityService != null) {
                ...
                Log.e(TAG, "getRootNodeInfo: nodeInfo: " + nodeInfo);
            }
...
//            nodeInfo = curEvent.getSource();
    ...
    private boolean performClick(List<AccessibilityNodeInfo> nodeInfos) {
        if (nodeInfos != null && !nodeInfos.isEmpty()) {
            ...
            for (int i = 0; i < nodeInfos.size(); i++) {
                ...
                // 获得点击View的类型
                Log.e(TAG, "performClick:  View类型：" + node.getClassName() );
                // 进行模拟点击
                if (node.isEnabled()) {
    ...
    private boolean performGlobalAction(int action) {
        try {
            Log.e(TAG, "performGlobalAction: isServiceRunning:"+isServiceRunning() );
            Log.e(TAG, "performGlobalAction: mAccessibilityService==null? "+(mAccessibilityService==null) );
